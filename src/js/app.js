...
function simulateInvestment() {
  ...
  document.getElementById('invCards').innerHTML = `
    ...
    <article class="stat"><span class="stat-label">${comIR ? 'Líquido est. (IR 15% ganho)' : 'Líquido = bruto'}</span><strong>${money.format(liquido)}</strong></article>
    <article class="stat"><span class="stat-label">Saldo p/ render R$ 1.000/mês</span><strong>${capital1000 == null ? '—' : money.format(capital1000)}</strong></article>
  `;
